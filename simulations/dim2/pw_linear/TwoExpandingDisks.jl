...
function main(; visualize = true, test = false)
	...
	chi_disk1 = construct_2D_indicator_fun("Disk"; center = [-1/2, 0], size = 1/3)

	chi_disk2 = construct_2D_indicator_fun("Disk"; center = [1/2, 0], size = 1/3)

	chi_twodisks(x,y) = chi_disk1(x,y) || chi_disk2(x,y)
	...
	if visualize
		p = GridVisualizer(; Plotter = Plots, layout = (2, 3), legend =:rt, fast = true, size = (1500, 600), title = "Expanding Disk with Saturated (top) and Unsaturated (bottom) Phase")
		n_t = length(tu_sat.t)
