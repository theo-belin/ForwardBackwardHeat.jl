...
function main(;type = "Smooth", visualize = true, test = false, p = 2)
	...
	K, lp_error_PWL, l2h1_error_PWL = convergence_error(problem_data_PWL; type)
	...
	if visualize
		...
		scalarplot!(
			p[1, 1], log10.(K.^(-1)), log10.(lp_error_PWL.^(-1)); label = "PWL", color = :blue,  
			title = "Convergence error in L^2_{t,x}", xlabel = "-log(dt))", ylabel = "-log-Error",
			markershape = :circle, markevery = 1
		)
		scalarplot!(p[1, 1], log10.(K.^(-1)), log10.(lp_error_Par.^(-1)); label = "Par", color = :green,clear = false)
		scalarplot!(p[1, 1], log10.(K.^(-1)), log10.(lp_error_Cub.^(-1)); label = "Cub", color = :red, clear = false)
		scalarplot!(
			p[1, 2], log10.(K.^(-1)), log10.(l2h1_error_PWL.^(-1)); label = "PWL", color = :blue,
			title = "Convergence error in L^2_tH^1_x", xlabel = "-log(dt)", ylabel = "-log-Error", 
			markershape = :circle, markevery = 1
		)
		scalarplot!(p[1, 2], log10.(K.^(-1)), log10.(l2h1_error_Par.^(-1)); label = "Par", color = :green, clear = false)
		scalarplot!(p[1, 2], log10.(K.^(-1)), log10.(l2h1_error_Cub.^(-1)); label = "Cub", color = :red, clear = false)
		reveal(p)
	end
